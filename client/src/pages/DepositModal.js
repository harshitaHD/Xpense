...
function DepositModal({ showDepositModal, setShowDepositModal, reloadData }) {
  ...
  const handleDeposit = async () => {
    try {
      ...
      // Check if the entered account number matches the ID of the logged-in user
      if (values.receiver !== user._id) {
        message.error("You can only deposit funds into your own account");
        return;
      }
